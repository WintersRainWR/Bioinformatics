...
输入：
```
grep -v '^#' 1.gtf | awk -F'\t' '$1=="XI" && $3=="CDS"' | sort -k5,5n | tail -10
```
...
输入：
```
grep -v '^#' 1.gtf | awk -F'\t' '$1=="IV"{print$3}' | sort | uniq -c | sort -n
```
...
输入：
```
grep -v '^#' 1.gtf | awk -F'\t' '$1!="IV" && $3=="CDS" && $7=="-" {print $5-$4+1}' | sort -nr | head -2
```
...
输入：
```
grep -v '^#' 1.gtf | awk -F'\t' '$1=="XV" && $3=="gene"{len=$5-$4+1; split($9,a,"\""); print a[2],len}' | sort -k2,2nr | head -5
```
...
输入：
```
grep -v '^#' 1.gtf | awk -F'\t' '{print NF; exit}'
```
